...
def long_time_task_step4(start_point, step_length):
    ...
    model_path = ddir + 'Camelyon_dla34up_bn_round1_exp0/Camelyon_dla34up_bn_latest.pth.tar'
    
    
    print('Run task %s (%s)...' % (start_point, os.getpid()))
    order = 'srun --mpi=pmi2 -p MIA -n1 --gres=gpu:1 -x SH-IDC1-10-5-30-219 --job-name Camelyon_Infer --ntasks-per-node=1 python -u Stage1_E_step_generate_pseudo_label.py \
	%d %d %s train_pos_1_wsi.txt,train_pos_2_wsi.txt,train_pos_3_wsi.txt,train_neg_wsi.txt,pos16wsi.txt,neg16wsi.txt \
    round1_exp0 '%(start_point, step_length,model_path,)
    print(order)
    os.system(order)
