    ...
    def __call__(self, image, label=None):
        ...
        if isinstance(self.size, int):
            ...
            if w < h:
                ...
                image = image.resize((ow, oh), self.image_interpolation)
                if label is not None:
                    label = label.resize((ow, oh), self.label_interpolation)
            else:
                oh = self.size
            ...
            image = image.resize(self.size[::-1], self.image_interpolation)
            if label is not None:
                label = label.resize(self.size[::-1], self.label_interpolation)
